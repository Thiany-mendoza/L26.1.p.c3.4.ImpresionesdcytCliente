...
export default class Cl_sPedidos {
  ...
  static async getPedidos(): Promise<{
    ok: boolean;
    tabla: Cl_mPedido[];
  }> {
    ...
    if (!res.ok) {
      ...
      if (local) {
        ...
        return { ok: true, tabla: parsed.map((p: any) => new Cl_mPedido(p)) };
      }
      // Si no hay local, inicializar con por defecto
      const mockPedidos = this.defaultPedidos.map((p: any) => new Cl_mPedido(p));
    ...
    let pedidos = res.tabla
      .filter((p: any) => p.tabla === "pedido" || p.idPedido)
      .map((p: any) => new Cl_mPedido({
        idPedido: p.idPedido,
  ...
  static async agregar(
    nuevoPedido: Cl_mPedido
  ): Promise<{ ok: boolean; mensaje: string; idPedido: string }> {
